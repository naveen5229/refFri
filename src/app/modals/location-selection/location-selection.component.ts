...
updateLocation(autocomplete) {
  ...
  if (city != null && district != null && state != null) {
    ...
    this.zone.run(() => {
      this.location.name = city + "," + district + "," + state;
    ...
    this.zone.run(() => {
      ...
      place.address_components.forEach(element => {
        if (element['types'][0] == "sublocality_level_1") {
          this.city = element.long_name;
        } else if (element['types'][0] == "sublocality") {
          this.city = element.long_name;
      ...
      this.location.name = this.city + "," + this.district + "," + this.state;
      if (!this.city || !this.district || !this.state) {
        console.log("test");
        this.resetData();
      }
      console.log(this.location);
