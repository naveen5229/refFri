...
@Component({
  ...
  styleUrls: ['./financialhistorysummary.component.scss']
})
...
ngOnInit(): void {
  ...
  //   this.maxDate = new Date(endDate.setFullYear(endDate.getFullYear() - 2));
  //   if(this.maxDate<this.startDate){
  ...
  getaddTimeFinancialTollReport() {
    ...
    if (this.mobileno) {
      let params = "&startDate=" + this.common.dateFormatter(new Date(this.startDate)) + "&endDate=" + this.common.dateFormatter(new Date(this.endDate)) + "&mobNo=" + this.mobileno + "&vehid=" + this.vehId;
      this.common.loading++;
      this.api.walle8Get('Tolls/getFoTollTxnSummary.json?' + params)
        .subscribe(res => {
          this.common.loading--;
