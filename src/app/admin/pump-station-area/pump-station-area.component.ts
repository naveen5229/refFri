...
@Component({
  ...
  styleUrls: ['./pump-station-area.component.scss', '../../pages/pages.component.css']
})
export class PumpStationAreaComponent implements OnInit {
...
ngOnInit() {
  ...
  gotoSingle(isClear = true) {
    this.commonService.loading++;
    this.apiService.post("SiteFencing/getPumpStationArea", { lat: this.locLatLng.lat, lng: this.locLatLng.lng })
      .subscribe(res => {
        let data = res['data'];
        if (!data) {
          this.isUpdate = false;
  ...
  DeleteArea() {
    ...
    this.apiService.post("SiteFencing/deletePumpStationArea", { locName: this.selectedArea.loc_name })
      .subscribe(res => {
        let data = res['data'];
        console.log('Res: ', res['data']);
