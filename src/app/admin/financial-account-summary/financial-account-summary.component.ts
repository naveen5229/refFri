...
export class FinancialAccountSummaryComponent implements OnInit {
  dates = {
    currentdate:this.common.dateFormatter(new Date()),
    start: null,
  ...
  calculateAmount(data)
  {
    for(let i=0;i<data.length;i++){
			let usageStatus = data[i]['entry_type'];
			let usageAmount = data[i]['amount'];
			if(usageStatus.toLowerCase()!="balance recharge")
			{
				this.debitAmount += usageAmount;
			}
			else{
				this.creditAmount += usageAmount;
			}
		}
  }
  getDate(date) {
