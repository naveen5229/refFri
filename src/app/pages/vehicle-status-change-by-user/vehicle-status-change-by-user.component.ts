...
@Component({
  ...
  styleUrls: ['./vehicle-status-change-by-user.component.scss']
})
export class VehicleStatusChangeByUserComponent implements OnInit {
  headings = [];
  valobj = {};
  table = {
    data: {
  ...
  getVehicleStatusAlerts() {
    this.common.loading++;
    this.api.get('HaltOperations/getFoAdminWiseVehicleStatus?')
      .subscribe(res => {
        this.common.loading--;
