...
@Component({
  ...
  styleUrls: ["./dashboard.component.scss", "../../pages/pages.component.css", "print.scss"],
  host: {
  ...
  getKPIS(isRefresh?) {
    ...
    this.api.get("VehicleKpis").subscribe(
      res => {
        ...
        if (res['code'] == 1) {
          res['data'] = res['data'].filter(kpi => {
            const secStates = [301, 311, 321];
            if (secStates.indexOf(kpi.x_sec_status) !== -1) {
              return true;
            }
            return false;
          });
