...
@Component({
  ...
  <div *ngIf="active">
    <div *ngFor="let d of data let i = index">
      ...
      <div *ngIf="!d.name"  (keydown)="keyHandler($event)" style="cursor:pointer" class="row x-warehouse"  (dblclick)="(d.y_voucher_type_name.toLowerCase().includes('voucher'))  ? (d.y_voucher_type_name.toLowerCase().includes('trip')) ? action(d,'',d.y_voucher_type_name) :action(d.y_voucherid,d.y_code,d.y_voucher_type_name) : action(d.y_voucherid,'',d.y_voucher_type_name)" (click)="selectedRow = i" [ngClass]="{'highlight' : selectedRow == i }">
...
export class outStandingTreeComponent {
 ...
 const key = event.key.toLowerCase();
  ...
  if ((key.includes('arrowup') || key.includes('arrowdown')) && this.data.length) {
    ...
    else if (this.selectedRow != this.data.length - 1) this.selectedRow++;
